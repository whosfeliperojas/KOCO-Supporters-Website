// Processes the 3D "virtual mascot" sheet (docs/peko-raw/Peko.png) into
// transparent WebP pose frames for the Companion, replacing the previous set.
//
// The 3D render's white body is warm-tinted like the background, so a color
// flood fill can't separate them — we use AI background removal
// (@imgly/background-removal-node, local ONNX model, build-time only).
//
// Outputs (web/public/peko/):
//   peko-idle.webp, peko-side.webp, peko-wave.webp, peko-celebrate.webp
//   peko-meta.json  — auto-detected eye positions on the idle frame (in % of
//                     canvas) + face color, used by the blink overlay.
//
// Run from web/:  node scripts/process-mascot-3d.mjs
...
const INPUT = path.join(here, "..", "..", "docs", "peko-raw", "Peko.png");
const OUT_DIR = path.join(here, "..", "public", "peko");
const TMP = fs.mkdtempSync(path.join(os.tmpdir(), "peko3d-"));

// Crop boxes exclude the sheet's speech bubble, settings panel, title and sparkle
const POSES = [
  { name: "peko-side",      box: { left: 55,  top: 260, width: 280, height: 480 } },
  { name: "peko-idle",      box: { left: 335, top: 242, width: 330, height: 578 } },
  { name: "peko-wave",      box: { left: 640, top: 315, width: 205, height: 400 } },
  { name: "peko-celebrate", box: { left: 848, top: 265, width: 225, height: 320 }, alphaBoost: true },
];
